      ...
      <div className="main-area" style={{ flex: 1 }}>
        <div className="topbar">
          ...
          <div className="topbar-right">
            <span className="batch-tag">{ALL_ENTRIES.length} events</span>
            <button className="btn btn-ghost"><i className="ti ti-download" /> Export log</button>
          </div>
        </div>
        ...
        <div className="page-pad">
          ...
          <div className="tab-summary-grid audit-summary" style={{ gridTemplateColumns: 'repeat(4,1fr)', marginBottom: 16 }}>
            <div className="tab-summary-card">
              <span>Total events</span><strong>{ALL_ENTRIES.length}</strong>
            </div>
            <div className="tab-summary-card">
              <span>AI decisions</span>
              <strong className="red">{ALL_ENTRIES.filter(e => e.status === 'ai' || e.status === 'blocked').length}</strong>
            </div>
            <div className="tab-summary-card">
              <span>Squad payments</span>
              <strong className="green">{ALL_ENTRIES.filter(e => e.status === 'paid').length}</strong>
            </div>
            <div className="tab-summary-card">
              <span>Blocked actions</span>
              <strong className="red">{ALL_ENTRIES.filter(e => e.status === 'blocked').length}</strong>
            </div>
          </div>
            ...
            {FILTERS.map(f => (
              ...
              >
                ...
                <span className="filter-count">
                  {f === 'all' ? ALL_ENTRIES.length : ALL_ENTRIES.filter(e => e.status === f).length}
                </span>
              </div>
            ))}
          </div>
                ...
                <div className="audit-content">
                  <div className="audit-row-top">
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span className={`badge ${STATUS_LABELS[entry.status]?.cls || 'badge-gray'}`}>
